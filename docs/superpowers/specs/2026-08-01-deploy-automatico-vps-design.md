...
1. **Documentar o deploy** para que cada push ao GitHub atualize automaticamente
   a VPS (`<VPS_USER>@<VPS_IP>`, porta SSH **<SSH_PORT>**).
...
- VPS host `<VPS_IP>`, user `<VPS_USER>`, porta SSH **<SSH_PORT>**.
- Secrets: `VPS_HOST=<VPS_IP>`, `VPS_USER=<VPS_USER>`, `VPS_SSH_KEY` (ed25519),
  `VPS_PORT=<SSH_PORT>`, `VPS_APP_PATH=/opt/linuxdecamoes`.
- Comandos SSH com `-p <SSH_PORT>` e `ssh-keyscan -p <SSH_PORT>`.
...
- `ssh-keygen`/`ssh-keyscan` documentados com `-p <SSH_PORT>`.
